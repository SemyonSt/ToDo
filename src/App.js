...
function App() {
  ...
  const onAddList = (obj) => { // добавляем название новой задачи
    const newList = [ // создается массив
      ...lists, // с уже старыми данными
      obj, // и добавляем новые
    ];
    setLists(newList);
  };

  const onEditListTitle = (id, title) => { //Меняем заголовок задачи
